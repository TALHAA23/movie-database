...
export default function Frame({
  ...
  return (
    <div
      className={`relative ${frameHight} max-h-[600px] w-full px-2 py-3 flex flex-col bg-black/90 text-white`}
    >
      <h1 className="relative first-letter:uppercase w-fit bg-white/10 px-2 rounded  pl-4 text-3xl font-semibold tracking-wide">
        {frameTitle}
      </h1>
      {frameSubTitle && (
        <h3 className=" text-sm font-semibold uppercase">{frameSubTitle}</h3>
      )}
