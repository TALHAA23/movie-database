...
function passwordValidator(password) {
    ...
    if (password.length < 8) {
        ...
        throw errorThrower("Password should be at least 8 character long", HttpError.BadRequest);
    }
    // Check if password contains a digit
    if (!/\d/.test(password)) {
        //custom error
        console.log("Password should contain at least one digit");
        throw errorThrower("Password should contain at least one digit", HttpError.BadRequest);
    }
    // Check if password contains a lowercase letter
    if (!/[a-z]/.test(password)) {
        //custom error
        console.log("Password should contain at least one lowercase letter");
        throw errorThrower("Password should contain at least one lowercase letter", HttpError.BadRequest);
    }
    // Check if password contains an uppercase letter
    if (!/[A-Z]/.test(password)) {
        //custom error
        console.log("Password should contain at least one uppercase letter");
        throw errorThrower("Password should contain at least one uppercase letter", HttpError.BadRequest);
    }
    // Check if password contains a special character
    if (!/[!@#$%^&*]/.test(password)) {
        //custom error
        console.log("Password should contain at least one special character");
        throw errorThrower("Password should contain at least one special character", HttpError.BadRequest);
    }
...
function emailValidator(email) {
    ...
    if (!localPart) {
        ...
        throw errorThrower("Email should have a local part before @", HttpError.BadRequest);
    }
    // Check if domain part is empty
    if (!domainPart) {
        //custom error
        console.log("Email should have a domain part after @");
        throw errorThrower("Email should have a domain part after @", HttpError.BadRequest);
    }
    ...
    if (!mainDomain) {
        ...
        throw errorThrower("Domain part should have a main domain before", HttpError.BadRequest);
    }
    // Check if top-level domain is empty
    if (!topLevelDomain) {
        //custom error
        console.log("Domain part should have a top-level domain after .");
        throw errorThrower("Domain part should have a top-level domain after .", HttpError.BadRequest);
    }
    // If all checks pass, return true
    return true;
}
