...
const recommendations = async (req, res, next) => {
    try {
        const userUid = req?.cookies?.user_id ||
...
const manageMyMovies = async (req, res, next) => {
    try {
        const { markTo } = req.query;
        const userId = req.cookies.user_id;
        if (!markTo)
            throw errorThrower("Incomplete URL", HttpError.BadRequest);
        if (!userId)
            throw errorThrower("Invalid User ID", HttpError.Forbidden);
        const body = req.body;
        if (!("movieRef" in body))
...
const myProfileMovies = async (req, res, next) => {
...
const myContributions = async (req, res, next) => {
...
const genreFromMyfavorites = async (req, res, next) => {
    const { user_id } = req.cookies;
    try {
        console.log(user_id);
        if (!user_id)
...
export { recommendations, userInfo, manageMyMovies, myProfileMovies, myContributions, genreFromMyfavorites, };
