...
export default async function updateMovieByFillingMissingInfo(data: AnyObject) {
  ...
  try {
    ...
    for (let [key, value] of objectToArray(data)) (doc as any)[key] = value; //update

    await doc?.save({
      validateBeforeSave: false,
    });

    await updateUserAddOnesByCreatingNewOrPushToExisting(
      userId as string,
      data
    );
    return { success: true };
...
const getDoc = (actionOn: string, _id: string) =>
  actionOn == "movie" ? Movie.findById(_id) : Actor.findById(_id);

const validateKeysAgainstSchema = (actionOn: string, data: AnyObject) => {
  const keys = Object.keys(data);
  for (let key of keys)
    if (
      (actionOn == "movie" && MovieSchema.pathType(key) !== "real") ||
      (actionOn == "actor" && ActorSchema.pathType(key) !== "real")
    )
      throw errorThrower("Invalid Key catched", HttpError.UnprocessableEntity);
};

const updateUserAddOnesByCreatingNewOrPushToExisting = async (
  userId: string,
  data: { [key: string]: any }
) => {
  const { _id } = data;
  ...
  const findIndex = userAddOns?.findIndex(
    (addOn) => addOn.ref?.toString() == _id
  );
